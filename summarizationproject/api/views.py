...
class SummarizerApi(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if id is not None:
            stu = Summarizer.objects.get(id=id)
            serializer = SummarizerSerializer(stu)
            return JsonResponse(serializer.data, safe=False)
        stu = Summarizer.objects.all()
        serializer = SummarizerSerializer(stu, many=True)
        return JsonResponse(serializer.data, safe=False)
    ...
    def post(self, request, *args, **kwargs):
        ...
        stream = io.BytesIO(json_data)
        pythondata = JSONParser().parse(stream)
        serializer = SummarizerSerializer(data=pythondata)
    ...
    def put(self, request, *args, **kwargs):
        ...
        stu = Summarizer.objects.get(id=id)
        serializer = SummarizerSerializer(stu, data = pythondata, partial=True)
        if serializer.is_valid():
            serializer.save()
